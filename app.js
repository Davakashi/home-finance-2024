    ...
    return {
        ...
        tusviigUzuuleh: function (tusuv) {
            if (tusuv.tusuv > 0)
                document.querySelector(DOMstrings.budgetLabel).textContent =
                    "+ " + formatMoney(tusuv.tusuv);
            else {
                document.querySelector(DOMstrings.budgetLabel).textContent =
                    formatMoney(tusuv.tusuv);
            }
            document.querySelector(DOMstrings.incomeLabel).textContent =
                formatMoney(tusuv.totalInc);
            document.querySelector(DOMstrings.expenseLabel).textContent =
                "- " + formatMoney(tusuv.totalExp);
            document.querySelector(DOMstrings.percentageLabel).textContent =
                formatMoney(tusuv.huvi) + "%";
        },
        ...
        addListItem: function (item, type) {
            ...
            if (type === "inc") {
                ...
                html =
                    '<div class="item clearfix" id="exp-%id%"><div class="item__description">%DESC%</div><div class="right clearfix"><div class="item__value">- %VALUE%</div><div class="item__percentage">%PERCENTAGE%%</div><div class="item__delete"><button class="item__delete--btn"><i class="ion-ios-close-outline"></i></button></div></div></div>';
            }
            // Тэр Html дотроо орлого зарлага утгуудыг REPLACE ашиглан өөрчлөнө.
            html = html.replace("%id%", item.id);
            html = html.replace("%DESC%", item.description);
            html = html.replace("%VALUE%", formatMoney(item.value));
...
var financeController = (function () {
    ...
    Expense.prototype.calcPercentage = function (totalIncome) {
        if (totalIncome === 0) this.percentage = 0;
        else {
    ...
    return {
        tusuvTootsooloh: function () {
            // Нийт орлого зарлагын нийлбэрийг тооцоолно.
            calculateTotal("inc");
            calculateTotal("exp");
